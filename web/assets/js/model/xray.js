...
class QuicStreamSettings extends XrayCommonClass {
    constructor(security=DieMethods.NONE,
                key='', type='none') {
        super();
...
class Inbound extends XrayCommonClass {
    ...
    genDieLink(address='', remark='') {
        if (this.protocol !== Protocols.Die) {
            return '';
        }
        let network = this.stream.network;
        let type = 'none';
    ...
    genLink(address='', remark='') {
        switch (this.protocol) {
            ...
            case Protocols.DIE: return this.genDieLink(address, remark)
...
Inbound.Settings = class extends XrayCommonClass {
    ...
    static getSettings(protocol) {
        switch (protocol) {
            ...
            case Protocols.DIE: return new Inbound.DIESettings(protocol);
    ...
    static fromJson(protocol, json) {
        switch (protocol) {
            ...
            case Protocols.DIE: return Inbound.DIESettings.fromJson(json);
...
Inbound.DIESettings = class extends Inbound.Settings {
    constructor(protocol,
                DIEes=[new Inbound.DIESettings.Die()],
                disableInsecureEncryption=false) {
        super(protocol);
        this.DIEes = DIEes;
        this.disableInsecure = disableInsecureEncryption;
    ...
    indexOfDieById(id) {
        return this.DIEes.findIndex(die => die.id === id);
    }

    addDie(die) {
        if (this.indexOfDieById(die.id) >= 0) {
            return false;
        }
        this.DIEes.push(die);
    }
    ...
    delDie(die) {
        const i = this.indexOfDieById(die.id);
        if (i >= 0) {
            this.DIEes.splice(i, 1);
        }
    }

    static fromJson(json={}) {
        return new Inbound.DIESettings(
            Protocols.DIE,
            json.clients.map(client => Inbound.DIESettings.Die.fromJson(client)),
            ObjectUtil.isEmpty(json.disableInsecureEncryption) ? false : json.disableInsecureEncryption,
        );
    }
    ...
    toJson() {
        return {
            clients: Inbound.DIESettings.toJsonArray(this.DIEes),
            disableInsecureEncryption: this.disableInsecure,
        };
    }
};
Inbound.DIESettings.Die = class extends XrayCommonClass {
    constructor(id=RandomUtil.randomUUID(), alterId=0) {
        super();
        this.id = id;
        this.alterId = alterId;
    ...
    static fromJson(json={}) {
        return new Inbound.DIESettings.Die(
            json.id,
            json.alterId,
        );
    }
